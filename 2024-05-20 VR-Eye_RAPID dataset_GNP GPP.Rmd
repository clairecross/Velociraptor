...
VR.Eye.data$GNP.minus.GPP <- VR.Eye.data$GNP_sim - VR.Eye.data$GPP_sim

#color by GNP/GPP-like status; -1=GNP, 1=GPP, 0=neither
VR.Eye.data$status <- ifelse(VR.Eye.data$GNP_sim >= sim.thresholds[1] & #high GNP sim
                          VR.Eye.data$GPP_sim < sim.thresholds[2], -1, #low GPP sim
                          ifelse(VR.Eye.data$GNP_sim < sim.thresholds[1] & #low GNP sim
                                   VR.Eye.data$GPP_sim >= sim.thresholds[2], 1, 0)) #high GPP sim


#plot GPP-GNP sim by patient
patient.axis.plot <- ggplot(VR.Eye.data, aes(x=Patient_ID, y=GNP.minus.GPP, col=as.factor(status))) + 
...
#export
if (export == TRUE){
  png(paste0(og.output.path, '/GNP GPP sim axis/GNP GPP axis by patient.png'), height=3000, width=1500, res=200)
  print(patient.axis.plot)
  dev.off()
}
```
