...
pops.to.plot <- c(1, 5:14) #skip NK cells, plasma cells, and T cells
...
for (i in pops.to.plot) { 
  ...
  cell.type.color <- cell.type.palette.all[names(cell.type.labels.all[which(cell.type.labels.all==goal)][1])]
  print(paste0(goal, ': ', cell.type.color))
...
#export
png(paste0(og.output.path, strftime(Sys.time(), "%m_%d "), "VR-Eye similarity on slide ", sample.number, '-', ROI.number, ".png"),
