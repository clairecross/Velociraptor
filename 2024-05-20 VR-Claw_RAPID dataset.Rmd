...
file.path <- './all data with 10 tSNEs.RData'
```

```{r load data}
#load cell data
load('C:/Users/clacr/Box/Irish_Lab/Projects/VRPTR + Cell Sim Paper/VR-Eye/2023-07-26 GNP GPP stability in RAPID dataset/tSNE files/all data with 10 tSNEs.RData')
combined.patient.data <- to.save
rm(to.save)
...
OS.data.files <-  dir(path='./', pattern="*.csv")
...
transformed.data <- combined.patient.data %>% select(-contains('File_ID')) %>% 
                    select(-contains('tSNE')) %>% mutate_all(function(x) asinh(x/5))

# choose markers to use for variance calculation and transform
chosen.markers <- transformed.data %>% select(contains('(v2)'))
...
#plot tSNE axes
tsne.plot <- embedding.plot(df=tsne.data, idx1=2, idx2=1, tsne.title='RAPID cohort')
print(tsne.plot)
```
...
```{r VR-Claw}
#run VR-Claw on the RAPID tsne axes with different values of k
for (i in kval.lst[9]){
  start_time <- Sys.time()
  kval <- i
  print(paste0('---------------------------- k of ', i, ' ----------------------------'))
  #create output folder
  dir.create(paste0('./output files/k=', i))
  output.path <- paste0('./output files/k=', i, '/')
  ...
  #export
  save(num.patients, file=paste0(output.path, 'num patients_vectorized.RData'))
  save(cox.summary, file=paste0(output.path, 'cox summary_vectorized.RData'))
  
  #check time
  total_time <- Sys.time() - start_time
  time_diff <- paste(total_time, attr(total_time, 'units')) 
  print(time_diff)
  write.table(time_diff, file=paste0(output.path, 'time elapsed_k=', kval, '_vectorized.txt'))
}
```

```{r results}
og.wd <- getwd()
#minimum number of patients and cells per cluster
...
for (i in kval.lst){
  ...
  #create output folder
  dir.create(paste0('./output files/k=', i, '/unfiltered'))
  dir.create(paste0('./output files/k=', i, '/filtered'))
  output.path <- paste0('./output files/k=', i, '/')

  
  #load neighborhood survival data
  load(paste0(output.path, 'cox summary_vectorized.RData'))
  load(paste0(output.path, 'num patients_vectorized.RData'))
