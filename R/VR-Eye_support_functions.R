MEM_edited <- function(exp_data, 
  ...
  if (num_pops==1 & zero.ref==TRUE){ #if there's only one cluster we'll need to transpose since R automatically changes the data format
    object_list_labeled <- lapply(list(t(as.matrix(MAGpop[,1:length(marker_names)-1])),
                                       ...
                                       t(as.matrix(prescaled_MEM_matrix[,1:length(marker_names)-1]))),
                                  rename_table)
  }else{
    object_list_labeled <- lapply(list(as.matrix(MAGpop[,1:length(marker_names)-1]),
                                       ...
                                       as.matrix(prescaled_MEM_matrix[,1:length(marker_names)-1])),
                                  rename_table)
  }
  object_list_labeled[[7]] <- file_order
  
  # List all matrices for export
  all_values <- list("MAGpop" = object_list_labeled[1],"MAGref"=object_list_labeled[2],"IQRpop"=object_list_labeled[3],"IQRref"=object_list_labeled[4],"MEM_matrix"=object_list_labeled[5],"prescaled_MEM_matrix"=object_list_labeled[6], "File Order" = object_list_labeled[7])
